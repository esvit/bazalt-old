...
using('Framework.Vendors.elFinder');
...
class elFinderVolumeORM extends elFinderVolumeDriver
{
    ...
    protected function make($path, $name, $mime)
    {
        ...
        $file->access = $access;
        $fileName = CMS\Bazalt::uploadFilename($name . '.txt', 'files');
    ...
    {
        return $this->tmpPath . PATH_SEP . md5($path);
    }
    ...
    protected function _save($fp, $dir, $name, $mime, $w, $h)
    {
        ...
        if ($this->tmpPath) {
            ...
            $file->height = $h;
            $file->alias = Url::cleanUrl($name);
            $fileName = CMS\Bazalt::uploadFilename($tmp, 'files');
            copy($tmp, $fileName);
            $file->path = relativePath($fileName);
            $file->mimetype = $mime;
            if ($parent) {
    ...
    protected function tmbname($stat)
    {
        ...
        $path = $name{0} . $name{1} . PATH_SEP;
        $path .= $name{2} . $name{3} . PATH_SEP;

        if (!is_dir($this->tmbPath . PATH_SEP . $path) && !mkdir($this->tmbPath . PATH_SEP .$path, 0777, true)) {
