...
use Components\Shop\Model\Category;

/**
 * @uri /shop/categories
 * @uri /shop/categories/:category_id
 */
class Categories extends CMS\Webservice\Rest
{
    ...
    {
        $user = CMS\User::get();
        $category = Category::getShopRootCategory();
        if (!$category) {
    ...
    public function createOrMoveMenuElement()
    {
        ...
        if ($isMoving) {
            ...
            $newElement->title = __('New category', \Components\Shop\Component::getName());
    ...
    public function saveCategory()
    {
        ...
        $data->field('title')->validator('hasDefaultTranslate', function($value) use (&$category, $languages, $data) {
            foreach ($languages as $language) {
                ...
                if (!$category->alias) {
                    $category->alias = Url::cleanUrl(\Framework\System\Locale\Config::findLocaleByAlias($language->id)->translit($category->title));
                }
                $category->save();
            }
        ...
        $category->is_published = 1;//$data['is_published'] == 'true';
        $category->save();
