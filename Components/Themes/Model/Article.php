...
class Article extends Base\Article// implements CMS_Search_ISearchable
{
    ...
    public static function create()
    {
        ...
        if (!CMS\User::get()->isGuest()) {
            $newsitem->user_id = CMS\User::get()->id;
        }
        $newsitem->site_id = CMS\Bazalt::getSiteId();
        $newsitem->publish = 1;
        $newsitem->item_type = 0;
        $newsitem->is_top = 0;
        $newsitem->comments_number = 0;
//print_r($newsitem);exit;
        return $newsitem;
    }
    ...
    public function getUrl($withHost = false)
    {
        ...
        /*if ($this->company_id) {
            ...
            return Route::urlFor('News.CompanyArticle', array('region' => $regionAlias, 'companyAlias' => $company->getAlias(), 'companyId' => $this->company_id, 'id' => $this->id), $withHost);
        }*/
        if (empty($regionAlias)) {
            return Route::urlFor('News.Article.Region.Category', array('region' => 'world', 'category' => $this->Category->Elements, 'id' => $this->id), $withHost);
        } else {
            if ($regionAlias != 'vinnytsia') {
                $regionAlias = 'ukraine';
            }
            return Route::urlFor('News.Article.Region.Category', array('region' => $regionAlias, 'category' => $this->Category->Elements, 'id' => $this->id), $withHost);
        }
        return $regionAlias;
    }
    ...
    public static function getCollection($published = false, $category = null, $region = null, $siteId = null, $userId = null)
    {
        ...
        $q = ORM::select('Components\News\Model\Article n')
            //->innerJoin('Components\News\Model\ArticleLocale ref', array('id', 'n.id'))
    ...
    protected function processImage($matches)
    {
        ...
        $img = CMS_Image::getThumb($matches['img'], $size);
        if (!$img) {
            return $matches[0];
        }
    ...
    public static function getAuthors($siteId = null)
    {
        ...
        $q = ORM::select('CMS_Model_User u', 'u.*')
