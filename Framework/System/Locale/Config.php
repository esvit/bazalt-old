...
class Config
{
    ...
    public function configure($config)
    {
        ...
        if (isset($config['detection'])) {
            ...
            if (!empty($allow)) {
                ...
                foreach (self::$allowLocales as $k => $v) {
                    self::$allowLocales[trim($v)] = trim($v);
                }
            }
            if (!empty($deny)) {
                self::$denyLocales = explode(',', $deny);
                foreach (self::$denyLocales as $k => $v) {
                    self::$denyLocales[trim($v)] = trim($v);
                }
            }
    ...
    public static function setLocale($locale)
    {
        ...
        if (!$syslocale) {
            if (array_key_exists($locale, self::$fallbacks)) {
                ...
                if ($retlocale = self::setLocale($trylocale)) {
                    ...
                    self::_setCurrentLocale($retlocale);
        ...
        if (is_array(self::$last) || self::$last['locale'] != $locale) {
            self::$last = array(
                'locale'    => $locale,
                'language'  => $language,
                'syslocale' => $syslocale
    ...
    /**
     * @param string $alias
     * @return Locale_AbstractLanguage
     * @throws Exception
     */
    ...
    public static function allowLocale($locale)
    {
        $locale = trim($locale);
        self::$allowLocales[$locale] = $locale;
    }
