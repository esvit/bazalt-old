...
class ORM_Relation_NestedSet extends ORM_Relation_Abstract implements ORM_Interface_RelationMany
{
    ...
    public function moveAfter($element, $clone = false)
    {
        ...
        $qUpdateLeft = ORM\ORM::update($this->name);
        $qUpdateRight = ORM\ORM::update($this->name);
    ...
    public function analyze()
    {
        ...
        $q = ORM\ORM::select($this->name . ' c1', 'c1.*')
        ...
        $q = ORM\ORM::select($this->name, 'MAX(' . self::RIGHT_FIELDNAME . ') AS `max_rgt`, COUNT(*) * 2 AS `count`')
        ...
        $q = ORM\ORM::select($this->name . ' m1', 'm1.*')
                ->from($this->name . ' m2')
