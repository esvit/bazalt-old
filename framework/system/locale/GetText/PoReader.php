...
class Locale_GetText_PoReader extends Locale_GetText_AbstractReader
{
    /**
    ...
    public static function importFromFile($filename)
    {
        ...
        $cacheKey = __CLASS__ . $filename;
        $cache = Cache::Singleton()->getCache($cacheKey);
        if ($cache) {
            $file = $cache['file'];
            $file->setHeaders(self::makeHeaders($cache['header']));
            return $file;
        }
        ...
        Cache::Singleton()->setCache($cacheKey, array('file' => $file, 'header' => $header), Cache::TIME_DAY, array('po_file'));
    ...
    {
        $entry = new Locale_Translation_Entry();
