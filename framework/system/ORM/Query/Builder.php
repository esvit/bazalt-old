 ...
 * @version    $Revision: 133 $
 */ 
abstract class ORM_Query_Builder extends ORM_Query
{
    /**
    ...
    {
        $this->joins[] = new ORM_Query_Join($name, $conditions, ORM_Query_Join::LEFT_JOIN);
        return $this;
    }
    ...
    {
        $this->joins[] = new ORM_Query_Join($name, $conditions, ORM_Query_Join::INNER_JOIN);
        return $this;
    }
    ...
    {
        $this->joins[] = new ORM_Query_Join($name, $conditions, ORM_Query_Join::RIGHT_JOIN);
        return $this;
    }
    ...
    {
        $this->joins[] = new ORM_Query_Join($name, $conditions, ORM_Query_Join::OUTER_JOIN);
        return $this;
    }
    ...
    /**
     * Встановлює модель або моделі, для якої буде будуватись запит
     *
    ...
    public function from($name)
    {
        ...
        foreach ($from as $class) {
            if (is_string($class)) {
                ...
                $this->models []= $names[0];
                $tableName = ORM_BaseRecord::getTableName($names[0]);
                $connection = ORM_BaseRecord::getSQLConnectionNameByModel($names[0]);
                $this->connection(ORM_Connection_Manager::getConnection($connection));
            ...
            } else if ($class instanceof ORM_Query_Builder) {
                $sql = $class->buildSQL();
    ...
    public function set($o, $param = null)
    {
        ...
        if ($o instanceof ORM_Record && count($o->getColumns()) > 0) {
            $obj = $o;
            foreach ($obj->getColumns() as $column) {
                $fieldName = $column->name();
                if (($column->isPrimaryKey() || $column->isAutoIncrement()) && $this instanceof ORM_Query_Update) {
    ...
    {
        if ($items instanceof ORM_Query_Builder) {
