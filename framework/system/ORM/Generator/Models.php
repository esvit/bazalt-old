...
class ORM_Generator_Models extends Object
{
    ...
    */
    ...
    public function generateFromDb(ORM_Connection_Abstract $connection = null, $path, $table = null, $prefix = null)
    {
        ...
        try {
            $this->shema = $connection->getConnectionString()->getDatabase();
            ...
            foreach ($tables as $table) {
                ...
                if( !file_exists( $fileName ) ) {
                    $content = $this->generateFileContent($name, $tableName, $res['fields'], $res['keys']);            
    ...
    /**
    * Генерує контент файлів моделей
    */
    protected function generateFileContent($className, $tableName, $fields, $keys)
    {
     ...
     */
    public function getFileDocComment()
    {
        $content  = '/**' . "\n";
        $content .= ' * Data model' . "\n";
        $content .= ' *' . "\n";
    ...
    public function generateBaseFileContent($className, $tableName, $fields, $keys)
    {
        ...
        $content .= '    }' . "\n\n";
        // $content .= '    public function getByParams($params, $type = ORMRecord::ONE_RECORD, $limit = null)' . "\n";
        // $content .= '    {' . "\n";
        // $content .= '        return parent::getRecordByParams($params, $type, $limit, self::MODEL_NAME);' . "\n";
        // $content .= '    }' . "\n\n";
    ...
    protected function getRelationType($tableName, $relation)
    {
        ...
        if ($res && $res->constrName == $res->refColumn) {
            ...
            self::$relations[ $tabName ][$refModel] = 'new ORMRelationMany2Many(\'' . $refModel . '\', \'' . $relColumn . '\', \''.$refTable.'\', \''. $column .'\')';
        } else {
            ...
            if( $this->isUnique($relation->refTable, $relation->refColumn) ) {
                ...
                self::$relations[ $tabName ][$refTable] = 'new ORMRelationOne2One(\''.$refTable.'\', \'' . $relation->column . '\',  \''. $relColumn .'\')';
                ...
                if( $this->isUnique($tableName, $relation->refColumn) ) {
                    ...
                    self::$relations[ $refTable ][$tabName] = 'new ORMRelationOne2One(\''.$tabName.'\', \'' . $relColumn . '\',  \''. $relation->column .'\')';
                } else {
                    #Генеруємо зворотній звязок - ORMRelationOne2Many
                    self::$relations[ $refTable ][$tabName] = 'new ORMRelationOne2Many(\''.$tabName.'\', \'' . $relColumn . '\',  \''. $relation->column .'\')';
                }
            } else {
                #Генеруємо прямий звязок - ORMRelationOne2Many
                self::$relations[ $tabName ][$refTable] = 'new ORMRelationOne2Many(\''.$refTable.'\', \'' . $relation->column . '\', \''. $relColumn .'\')';
                #Генеруємо зворотній звязок - ORMRelationOne2One
                self::$relations[ $refTable ][$tabName] = 'new ORMRelationOne2One(\''.$tabName.'\', \'' . $relColumn . '\',  \''. $relation->column .'\')';
            }
        }        
