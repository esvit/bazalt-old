 ...
 * @version    $Revision: 133 $
 */ 
abstract class ORM_Relation_Abstract extends Object implements IEventable
{
    /**
     ...
     *
     * @param ORM_Record &$object поточний об'єкт
     *
     * @return void
     */
    public function setBaseObject(ORM_Record &$object)
    {
     ...
     * @return void
     */
    protected function checkType(ORM_Record $item)
    {
    ...
    protected function applyAddParams(ORM_Query $q)
    {
        if ($this->additionalParams) {
            foreach ($this->additionalParams as $name => $value) {
                if ($value === null) {
                    $q->andWhere('ft.' . $name . ' IS ?', null);
                } else {
                    $q->andWhere('ft.' . $name . ' = ?', $value);
                }
            }
        }    
    }
