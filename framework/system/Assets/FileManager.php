...
class Assets_FileManager implements Config_IConfigurable
{
    ...
    public static function &getInstance()
    {
        if (self::$instance == null) {
            ...
            Configuration::init('assets', self::$instance);
        }
    ...
    {
        $this->baseDir = isset($config['baseDir']) ? Configuration::replaceConstants($config['baseDir']) : null;

        $this->originalDir = isset($config['originalDir']) ? Configuration::replaceConstants($config['originalDir']) : null;

        $this->domain = isset($config['domain']) ? Configuration::replaceConstants($config['domain']) : null;
    ...
    {
        $fn = self::filename($file, STATIC_DIR . '/original');
        if (file_exists($fn) && !is_link($fn)) {
            throw new Exception('Permision denied for write file ' . $fn);
        }
