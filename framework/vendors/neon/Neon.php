...
class Neon
{
	...
	public static function encode($var, $options = NULL)
	{
		...
		} elseif ($var instanceof Neon_Entity) {
	...
	private function tokenize($input)
	{
		...
		if ($code = preg_last_error()) {
			throw new Neon_Exception('PREG error', $code);
		}
	...
	private function error($message = "Unexpected '%s'")
	{
		...
		throw new Neon_Exception(str_replace('%s', $token, $message) . " on line $line, column $col.");
	}
}