...
final class Event extends Object
{
     ...
     * Конструктор
     *
     * @param IEventable $obj       Об'єкт для якого інціалізується евент
     * @param string     $eventName Ім'я евента
     */ 
    protected function __construct(IEventable $obj, $eventName)
    {
     ...
     *
     * @param IEventable $object Ініціалізує евенти для об'єкту
     */
    public static function init(IEventable $object)
    {
     ...
     *
     * @param  IEventable $object    Об'єкт для якого ініціалізується евент
     * @param  string     $eventName Ім'я евента
     * @return Event
     */ 
    public static function initEvent(IEventable $object, $eventName)
    {
        $className = get_class($object);
     ...
     *
     * @param  IEventable $object Об'єкт з ініціалізованими евентами
     * @return array
     */ 
    public static function getEvents(IEventable $object)
    {
        $className = get_class($object);
     ...
     * @return Event|null
     */
    public static function get(IEventable $object, $eventName)
    {
     ...
     * @return boolean
     */
    public static function isValid(IEventable $object, $eventName)
    {
        $events = self::getEvents($object);
    ...
    {
        $key = array_search($callback, $this->events);
    ...
    public static function register($objName, $name, $callback)
    {
        ...
        } elseif (in_array($callback, self::$globalEvents[$objName][$name], true)) {
    ...
    public static function unregister($objName, $name, $callback)
    {
        ...
        $key = array_search($callback, self::$globalEvents[$objName][$name]);
