...
class Library
{
    ...
    public function __construct($namespace, $includeFile = self::INCLUDE_FILE)
    {
        if (!self::isValid($namespace)) {
            throw new Exception_Namespace('Invalid namespace "' . $namespace . '"');
        }

        $this->names = explode(self::SEPARATOR, $namespace);
        $this->namespaceString = $namespace;

        $this->path = DataType_String::replaceConstants($this->getPath());
    ...
    public function getFilename()
    {
        ...
        if (!$fileName) {
            Core_Autoload::registerNamespace($this->getAutoloadPrefix(), $path);
            return null;
        }
        return $fileName;
    }
